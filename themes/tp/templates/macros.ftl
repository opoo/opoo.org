...
<#if (parent.children?size > 0)>
    ...
    <#list parent.children as child>
        ...
        <li class="category2"><a href="${root_url}${child.url}"><span class="badge pull-right">${child.pages?size}</span>${child.name}</a>
...
<#macro showCategoryItem category level>
    <li class="padding-left-level-${level}">
        <a href="${root_url}${category.url!'#'}"><span class="badge pull-right">${category.pages?size}</span>${category.name}</a></li>
